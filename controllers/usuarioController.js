...
export const getUsuarioByEmail= async (req, res, next) => {
    try {
        const [rows] = await connection.execute('SELECT * FROM usuarios WHERE email = ?', [req.params.email]);
        const usuario = rows[0];
        if (usuario) {
            res.json(usuario);
        } else {
...
export const createUsuario = async (req, res, next) => {
    try {
        ...
        // Verificar si el nombre, email o teléfono ya existen
        const [existingUsers] = await connection.execute(
            'SELECT * FROM usuarios WHERE email = ? OR phone = ?',
            [email, phone]
        );
